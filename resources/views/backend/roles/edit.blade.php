...
@section('title', trans('navigation.backend.access.roles.edit'))
...
@section('content')
    {{ Form::model($role, ['route' => ['backend.roles.update', $role], 'class' => 'form-horizontal', 'role' => 'form', 'method' => 'PATCH']) }}

    <div class="row">
        <div class="col-12">
            <div class="card card-primary card-outline">
                <div class="card-header">
                    <h3 class="card-title">{{ trans('navigation.backend.access.roles.edit') }}</h3>
                    ...
                    <div class="form-group">
                        ...
                        @forelse($permissions as $permission)
                            <div class="form-check">
                                {{ Form::checkbox('associatedPermissions[]', $permission->id, $role->hasPermission($permission->key) ? 'checked' : '', ['class' => 'form-check-input', 'disabled' => $role->hasPermission($permission->key) && $role->isSuperAdmin(), 'id' => 'permission-' . $permission->key]) }}
